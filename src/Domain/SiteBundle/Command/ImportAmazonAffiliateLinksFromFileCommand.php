...
class ImportAmazonAffiliateLinksFromFileCommand extends ContainerAwareCommand
{
    ...
    private function importData($data)
    {
        ...
        foreach ($data as $row) {
            $categoryName = AdminHelper::convertAccentedString(trim(explode(self::CATEGORY_NAME_DELIMITER, $row['Category'])[0]));
            /** @var Category $category */
            $category = $em->getRepository(Category::class)->getCategoryByCaseInsensitiveName([$categoryName]);

            if (!$category) {
                throw new EntityNotFoundException('Category ' . $categoryName . ' not found');
            }
