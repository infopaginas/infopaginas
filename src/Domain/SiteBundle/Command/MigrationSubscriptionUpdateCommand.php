...
class MigrationSubscriptionUpdateCommand extends ContainerAwareCommand
{
    const DEFAULT_LOCALE  = 'en';

    const API_BASE_URL    = 'http://infopaginas.drxlive.com/api/businesses';

    const NO_DATA_ERROR   = 'NO_DATA_ERROR';
    ...
    /**
     * @var int $businessesUpdated
     */
    protected $businessesUpdated = 0;

    /**
     * @var int $superVmUpdated
     */
    protected $superVmUpdated = 0;
    ...
    {
        $this->setName('data:migration:subscription-update');
        $this->setDescription('Update subscriptions and super vm');
        $this->setDefinition(
            new InputDefinition([
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($this->withDebug) {
            ...
            $this->output->writeln($this->businessesUpdated . ' Businesses were updated');
            $this->output->writeln($this->superVmUpdated . ' SuperVm businesses were updated');

            if (!empty($page)) {
                $this->output->writeln('Page start ' . $pageStart);
                $this->output->writeln('Page finish ' . $page);
            }
        }
    }
